...
void Game::setGameState(GameState_t newState)
{
	...
	switch (newState) {
		case GAME_STATE_INIT: {
			...
			loadBoostBoss();
			loadBoostMonster();
			loadBoostMonsterSecond();
			loadBoostMonsterThird();
			groups.load();
			g_chat->load();
						   ...
						   uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count)
{
	...
	if (Creature* movingCreature = thing->getCreature()) {
		...
		if (!tile) {
			...
			//AQUI ALTERA A VELOCIDADE DO PUSH DEFAULT 1200
...
bool Game::playerSaySpell(Player* player, SpeakClasses type, const std::string& text)
{
	...
	if (result == TALKACTION_BREAK) {
		...
		constexpr uint32_t STORAGE_HIDDEN_SPELLS = 256331;

		int32_t hiddenValue;
		player->getStorageValue(STORAGE_HIDDEN_SPELLS, hiddenValue);

		if (hiddenValue == 1) {
			return true; // não envia nenhuma mensagem no chat
		}
							   ...
							   bool ghostMode, SpectatorHashSet* spectatorsPtr/* = nullptr*/, const Position* pos/* = nullptr*/)
{
	...
	// Enviar para os jogadores visíveis
	for (Creature* spectator : ((isSpectatorsPtrInvalid) ? spectators : *spectatorsPtr)) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			// Pular se o player está no modo oculto
			int32_t hiddenValue;
			tmpPlayer->getStorageValue(STORAGE_HIDDEN_SPELLS, hiddenValue);
			if (hiddenValue == 1) {
				continue; // não envia a mensagem para este jogador
			}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		if (realManaChange > 0 && !target->isInGhostMode()) {
			...
			message.primary.color = TEXTCOLOR_PURPLE;
...
void Game::playerSendThankYou(uint32_t playerId, uint32_t a_statementId)
{
	...
	/*std::string fileName = "data/logs/thankyou/" + name + ".txt"; // AQUI É SOBRE LOGS TAMBÉM.
...
void Game::playerPreyAction(uint32_t playerId, uint8_t preySlotId, PreyAction_t preyAction, uint8_t monsterIndex, uint16_t raceId)
{
	...
	player->setPreyExausted(OS_TIME(nullptr) + 2);
...
bool Game::loadBoostBoss()
{
	Database& db = Database::getInstance();
	DBResult_ptr resultQuery1 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_boss'");
	if (!resultQuery1) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_boss', '1')");
		boostBossId = 1;
	} else {
		boostBossId = resultQuery1->getNumber<uint16_t>("value");
	}

	DBResult_ptr resultQuery2 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_boss_name'");
	if (!resultQuery2) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_boss_name', 'none')");
		boostBossName = "none";
	} else {
		boostBossName = resultQuery2->getString("value");
	}

	DBResult_ptr resultQuery3 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_boss_url'");
	if (!resultQuery3) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_boss_url', 'none')");
	}

	return true;
}

void Game::setBoostBoss(std::string Bossname, uint16_t Bossid)
{
	Database& db = Database::getInstance();
	std::ostringstream query;
	query << "UPDATE `server_config` SET `value` = '" << Bossid << "' WHERE `config` = 'boost_boss'";
	db.executeQuery(query.str());

	query.str(std::string());
	query << "UPDATE `server_config` SET `value` = " << db.escapeString(Bossname) << " WHERE `config` = 'boost_boss_name'";
	db.executeQuery(query.str());

	// seting outfit
	MonsterType* monsterType = g_monsters.getMonsterTypeByRace(Bossid);
	...
	query.str(std::string());
	query << "UPDATE `server_config` SET `value` = " << db.escapeString(outfitstr.str()) << " WHERE `config` = 'boost_boss_url'";
	db.executeQuery(query.str());
...
bool Game::loadBoostMonsterSecond()
{
	Database& db = Database::getInstance();
	DBResult_ptr resultQuery1 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_monsterSecond'");
	if (!resultQuery1) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_monsterSecond', '1')");
		boostRace = 1;
	} else {
		boostRaceSecond = resultQuery1->getNumber<uint16_t>("value");
	}

	DBResult_ptr resultQuery2 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_monster_nameSecond'");
	if (!resultQuery2) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_monster_nameSecond', 'none')");
		boostMonsterSecond = "none";
	} else {
		boostMonsterSecond = resultQuery2->getString("value");
	}

	DBResult_ptr resultQuery3 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_monster_urlSecond'");
	if (!resultQuery3) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_monster_urlSecond', 'none')");
	}

	return true;
}

void Game::setBoostMonsterSecond(std::string monsternameSecond, uint16_t monsteridSecond)
{
	Database& db = Database::getInstance();
	std::ostringstream query;
	query << "UPDATE `server_config` SET `value` = '" << monsteridSecond << "' WHERE `config` = 'boost_monsterSecond'";
	db.executeQuery(query.str());

	query.str(std::string());
	query << "UPDATE `server_config` SET `value` = " << db.escapeString(monsternameSecond) << " WHERE `config` = 'boost_monster_nameSecond'";
	db.executeQuery(query.str());

	// seting outfit
	MonsterType* monsterType = g_monsters.getMonsterTypeByRace(monsteridSecond);
	std::ostringstream outfitstr;
	if (monsterType) {
		Outfit_t outfit = monsterType->info.outfit;
		if (outfit.lookType > 0) {
	...
	query.str(std::string());
	query << "UPDATE `server_config` SET `value` = " << db.escapeString(outfitstr.str()) << " WHERE `config` = 'boost_monster_urlSecond'";
	db.executeQuery(query.str());

	boostRaceSecond = monsteridSecond;
	boostMonsterSecond = monsternameSecond;
}

bool Game::loadBoostMonsterThird()
{
	Database& db = Database::getInstance();
	DBResult_ptr resultQuery1 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_monsterThird'");
	if (!resultQuery1) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_monsterThird', '1')");
		boostRace = 1;
	} else {
		boostRaceThird = resultQuery1->getNumber<uint16_t>("value");
	}

	DBResult_ptr resultQuery2 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_monster_nameThird'");
	if (!resultQuery2) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_monster_nameThird', 'none')");
		boostMonsterThird = "none";
	} else {
		boostMonsterThird = resultQuery2->getString("value");
	}

	DBResult_ptr resultQuery3 = db.storeQuery("SELECT `value` FROM `server_config` WHERE `config` = 'boost_monster_urlThird'");
	if (!resultQuery3) {
		db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES ('boost_monster_urlThird', 'none')");
	}

	return true;
}

void Game::setBoostMonsterThird(std::string monsternameThird, uint16_t monsteridThird)
{
	Database& db = Database::getInstance();
	std::ostringstream query;
	query << "UPDATE `server_config` SET `value` = '" << monsteridThird << "' WHERE `config` = 'boost_monsterThird'";
	db.executeQuery(query.str());

	query.str(std::string());
	query << "UPDATE `server_config` SET `value` = " << db.escapeString(monsternameThird) << " WHERE `config` = 'boost_monster_nameThird'";
	db.executeQuery(query.str());

	// seting outfit
	MonsterType* monsterType = g_monsters.getMonsterTypeByRace(monsteridThird);
	std::ostringstream outfitstr;
	if (monsterType) {
		Outfit_t outfit = monsterType->info.outfit;
		if (outfit.lookType > 0) {
	...
	query.str(std::string());
	query << "UPDATE `server_config` SET `value` = " << db.escapeString(outfitstr.str()) << " WHERE `config` = 'boost_monster_urlThird'";
	db.executeQuery(query.str());

	boostRaceThird = monsteridThird;
	boostMonsterThird = monsternameThird;
}
...
void Game::saveServeMessage()
{
	...
	// Removendo oque gera os logs (msgs de players)
	/*
