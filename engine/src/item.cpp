		...
		if (it.abilities) {
			...
			if (!show) {
				...
				for (size_t i = 0; i < COMBAT_COUNT; ++i) {
					...
					// Check if this element has dynamic bonus and combine valuesAdd commentMore actions
					CombatType_t combatType = indexToCombatType(i);
				...
				// Now show ONLY dynamic absorptions that don't have static counterpartsAdd commentMore actions
				if (item) {
