...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/) const
{
    ...
    // ✅ DANO ENTRE 30% E 100% DO VALOR MÁXIMO
    int32_t minDamage = static_cast<int32_t>(maxValue * 0.3);
    return -normal_random(minDamage, maxValue);
}
...
int32_t WeaponDistance::getElementDamage(const Player* player, const Creature* target, const Item* item) const
{
	...
	//  GARANTIR QUE O DANO MÍNIMO SEJA PELO MENOS 30% DO MÁXIMO
	int32_t minThreshold = static_cast<int32_t>(maxValue * 0.5);
...
int32_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item, bool maxDamage /*= false*/) const
{
	...
	if (target) {
		if (target->getPlayer()) {
			// ✅ CORREÇÃO PvP: Garantir mínimo de 50% do dano máximo contra players
			minValue = static_cast<int32_t>(std::ceil(player->getLevel() * 0.1));
			int32_t minGuaranteed = static_cast<int32_t>(maxValue * 0.35); // 50% do máximo
			minValue = std::max(minValue, minGuaranteed); // Usar o maior valor entre os dois
		} else {
			minValue = static_cast<int32_t>(std::ceil(player->getLevel() * 0.2));
		}
	} else {
