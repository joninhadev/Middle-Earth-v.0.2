function onModalWindow(player, modalWindowId, buttonId, choiceId)
	...
	if buttonId == 102 and modalWindowId == 1000 then
		...
		if modalWindowId == 1005 then
			...
			local bp2 = {}
			for i = 0, getContainerSize(getPlayerSlotItem(player, CONST_SLOT_STORE_INBOX).uid) do
				local thing = getContainerItem(getPlayerSlotItem(player, CONST_SLOT_STORE_INBOX).uid, i)
				container[i] = getContainerItem(getPlayerSlotItem(player, CONST_SLOT_STORE_INBOX).uid, i)
	...
	if param == 'add' then
		...
		for _, loot in pairs(lootBlockList) do
			...
			if not table.contains(check, ItemType(lootBlockList[sum].itemId):getName()) then
				...
				window:addChoice(sum, "".. string.gsub(" "..status..""..ItemType(lootBlockList[sum].itemId):getName(), "%W%l", string.upper):sub(2, 21) .." "..string.gsub(" "..string.lower(backvinculo), "%W%l", string.upper):sub(2).."")
			end
			sum = sum + 1
		end
		if autolootBP == 1 then
			window:addButton(105, "Loot Pouch")
		end
		...
		local message = "Choose the Loot Pouch:"
		local window = ModalWindow(modalcode, title, message)
		local sum = 1
		local container, names = {}, {}
		if getContainerSize(getPlayerSlotItem(player, CONST_SLOT_STORE_INBOX).uid) then
			for i = 0, getContainerSize(getPlayerSlotItem(player, CONST_SLOT_STORE_INBOX).uid) do
				local thing = getContainerItem(getPlayerSlotItem(player, CONST_SLOT_STORE_INBOX).uid, i)
				container[i] = getContainerItem(getPlayerSlotItem(player, CONST_SLOT_STORE_INBOX).uid, i)
				if isContainer(container[i].uid) then
