function onLogin(player)
	...
	local offlineTime = lastLogout ~= 0 and math.min(os.stime() - lastLogout, 86400 * 21) or 0
	...
	local regainStaminaMinutes = offlineTime / 90
	if regainStaminaMinutes > maxNormalStaminaRegen then
		local happyHourStaminaRegen = (offlineTime - (maxNormalStaminaRegen * 180)) / 300
