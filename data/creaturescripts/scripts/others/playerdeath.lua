...
function onDeath(player, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
    ...
    if AutoLootList and AutoLootList.onLogout then
    ...
    if byPlayer == 1 then
        ...
        -- REMOVIDO: Trecho problemático que causava o erro na linha 138
                        ...
                        db.asyncStoreQuery("SELECT COUNT(*) as 'count' FROM `guildwar_kills` WHERE warid = ".. warid .." AND `killerguild` = " .. guildid .. ";",function(query)
                            if(query) then
                                ...
                                if count >= frags then
                                    ...
                                    -- CORREÇÃO: Verificar se as guilds existem antes de chamar broadcastMessage
                                    local guild1 = Guild(guildid)
                                    local guild2 = Guild(guildid2)
                                    
                                    if guild1 and guild1.broadcastMessage then
                                        guild1:broadcastMessage("War is over, please relog", MESSAGE_EVENT_ADVANCE)
                                    end
                                    if guild2 and guild2.broadcastMessage then
                                        guild2:broadcastMessage("War is over, please relog", MESSAGE_EVENT_ADVANCE)
                                    end
                                    
                                    -- CORREÇÃO: Obter os nomes das guilds de forma segura
                                    local guild1Name = guild1 and guild1:getName() or "Unknown Guild"
                                    local guild2Name = guild2 and guild2:getName() or "Unknown Guild"
                                    
                                    print(string.format("The war between '%s' and '%s' has ended.", guild1Name, guild2Name))
                                    Game.broadcastMessage(string.format("The war between '%s' and '%s' has ended. Winner: %s", guild1Name, guild2Name, guild1Name), MESSAGE_EVENT_ADVANCE)
                                end
                            end
                        end)
