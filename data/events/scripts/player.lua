...
function Player:onMoveCreature(creature, fromPosition, toPosition)
   if hasEvent.onMoveCreature then
    ...
    if self:getGroup():getId() < 4 then
        ...
        if creature:isPlayer() and creature:getStorageValue(Storage.isTraining) > 0 then
            self:sendCancelMessage("O jogador não pode ser puxado enquanto está treinando com uma exercise weapon.")
            return false
        end
    end
    return true
