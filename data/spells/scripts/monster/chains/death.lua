local dmgType = COMBAT_DEATHDAMAGE
local effect = 18
local minDmg = 550
local maxDmg = 1000
...
local function chain(creature)
	...
	while (totalChain < 5 and #targets > 0) do
		...
		if updateLastChain then
			doTargetCombatHealth(0, closestMonster, COMBAT_DEATHDAMAGE, -minDmg, -maxDmg, CONST_ME_NONE)
...
function onCastSpell(creature, variant)
	...
	if target then
		...
		doTargetCombatHealth(0, target, COMBAT_DEATHDAMAGE, -minDmg, -maxDmg, CONST_ME_NONE)
		if total > 0 then
			 return true
		else
