local dmgType = COMBAT_ENERGYDAMAGE
local effect = 177
local minDmg = 1300
local maxDmg = 1500
...
local function chain(creature)
	...
	while (totalChain < 5 and #targets > 0) do
		...
		if updateLastChain then
			doTargetCombatHealth(0, closestMonster, COMBAT_ENERGYDAMAGE, -minDmg, -maxDmg, CONST_ME_NONE)
...
function onCastSpell(creature, variant)
	...
	if target then
		...
		doTargetCombatHealth(0, target, COMBAT_ENERGYDAMAGE, -minDmg, -maxDmg, CONST_ME_NONE)
		if total > 0 then
			 return true
		else
