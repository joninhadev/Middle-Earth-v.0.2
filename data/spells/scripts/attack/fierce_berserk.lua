...
function onGetFormulaValues(player, skill, attack, factor, tool)
	...
	local min, max = player:getSpellDamage(-(((skillTotal * 0.12) + 12) + (levelTotal)), -(((skillTotal * 0.15) + 27) + (levelTotal)))

	local skillSecond = skill * element
	local minb, maxb = 0, 0
	if skillSecond > 0 then
		minb, maxb = player:getSpellDamage(-(((skillSecond * 0.12) + 12) + (levelTotal)), -(((skillSecond * 0.15) + 27) + (levelTotal)))
	end
