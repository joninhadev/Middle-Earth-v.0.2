...
function onGetFormulaValues(player, skill, attack, factor, tool)
	...
	local min, max = player:getSpellDamage(-(((skillTotal * 0.07) + 7) + (levelTotal)), -(((skillTotal * 0.09) + 11) + (levelTotal)))

	local skillSecond = skill * element
	local minb, maxb = 0, 0
	if skillSecond > 0 then
		minb, maxb = player:getSpellDamage(-(((skillSecond * 0.07) + 7) + (levelTotal)), -(((skillSecond * 0.09) + 11) + (levelTotal)))
	end
