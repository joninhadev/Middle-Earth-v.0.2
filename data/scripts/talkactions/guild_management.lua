--[[
    ...
    Complete guild management solution with professional features:
...
function checkInvitations.onSay(player, words, param, type)
    ...
    -- Get all pending invitations for this guild
    local query = db.storeQuery(string.format([[
        SELECT 
            gi.id, 
            p.name, 
    ...
    -- Display pending invitations in a popup
...
function onlineMembers.onSay(player, words, param, type)
    ...
    -- Get all guild members who are online
    local onlineCount = 0
    ...
    GuildUtils.sendMessage(player, onlineText, nil, true) -- Use popup
    return false
end
...
function guildStats.onSay(player, words, param, type)
    ...
    -- Display guild statistics in a popup
    ...
    -- Additional advanced statistics could be added here
