local energyLibrary = CreatureEvent("energyLibrary")

local config = {
     ['warlock'] = {amount = 1000, storage = 38125, startstorage = 6232, startvalue = 1},
     ['wyrm'] = {amount = 1000, storage = 38126, startstorage = 6232, startvalue = 1},
     ['elder wyrm'] = {amount = 1000, storage = 38127, startstorage = 6232, startvalue = 1}
}

-- Tabela para armazenar o índice do próximo jogador para cada party
local partyRotation = {}

function energyLibrary.onKill(creature, target)
     ...
     if currentStorage < monster.amount then
         ...
         if newValue < monster.amount then
             selectedPlayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, '[Acesso a livraria de energy]: '.. newValue ..' de '.. monster.amount ..' '.. monsterName ..'s mortos.')
         else
