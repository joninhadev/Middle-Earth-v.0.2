local iceLibrary = CreatureEvent("iceLibrary")

local config = {
     ['ice witch'] = {amount = 1000, storage = 38122, startstorage = 6231, startvalue = 1},
     ['frost dragon'] = {amount = 1000, storage = 38123, startstorage = 6231, startvalue = 1},
     ['burster spectre'] = {amount = 1000, storage = 38124, startstorage = 6231, startvalue = 1}
}

-- Tabela para armazenar o índice do próximo jogador para cada party
local partyRotation = {}

function iceLibrary.onKill(creature, target)
     ...
     if currentStorage < monster.amount then
         ...
         if newValue < monster.amount then
             selectedPlayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, '[Acesso a livraria de ice]: '.. newValue ..' de '.. monster.amount ..' '.. monsterName ..'s mortos.')
         else
