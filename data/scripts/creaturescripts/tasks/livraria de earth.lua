local earthLibrary = CreatureEvent("earthLibrary")

local config = {
     ['hydra'] = {amount = 1000, storage = 38128, startstorage = 6233, startvalue = 1},
     ['serpent spawn'] = {amount = 1000, storage = 38129, startstorage = 6233, startvalue = 1},
     ['medusa'] = {amount = 1000, storage = 38130, startstorage = 6233, startvalue = 1}
}

-- Tabela para armazenar o índice do próximo jogador para cada party
local partyRotation = {}

function earthLibrary.onKill(creature, target)
     ...
     if currentStorage < monster.amount then
         ...
         if newValue < monster.amount then
             selectedPlayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, '[Acesso a livraria de earth]: '.. newValue ..' de '.. monster.amount ..' '.. monsterName ..'s mortos.')
         else
