-- Global variables and functions needed for onTargetCombat
__picif = __picif or {}
...
local staminaBonus = {
	target = 'Treinador',
	period = 300000, -- time in milliseconds
...
local function addStamina(name)
	...
	if not player then
		...
		if not target or target:getName() ~= staminaBonus.target then
			...
			local oldStamina = player:getStamina()
			player:setStamina(oldStamina + staminaBonus.bonus)
			local newStamina = player:getStamina()
			player:sendTextMessage(MESSAGE_STATUS_DEFAULT, "Você ganhou " .. staminaBonus.bonus .. " stamina! (" .. newStamina .. "/2520)")
...
local event = Event()
event.onTargetCombat = function(self, target)
	...
	if self:isPlayer() then
		if target and target:getName() == staminaBonus.target then
			...
			if not staminaBonus.events[name] then
				self:sendTextMessage(MESSAGE_STATUS_DEFAULT, "[Stamina Recovery] Você receberá " .. staminaBonus.bonus .. " de stamina a cada " .. (staminaBonus.period/1000) .. " segundos.")
				staminaBonus.events[name] = addEvent(addStamina, staminaBonus.period, name)
			end
		end
	end
