...
event.onGainExperience = function(self, source, exp, rawExp, sendText)
	...
	-- Guild House Domination Bonus (5% extra exp)
	local guild = self:getGuild()

	if guild then -- Verifica se o jogador tem uma guild
		local resultId = db.storeQuery(string.format(
			"SELECT guild FROM gh WHERE guild = %d LIMIT 1",
			guild:getId()
		))

		if resultId then
			exp = exp * 1.05 -- 5% extra experience for guild members if their guild dominates the Guild House
			result.free(resultId)
			self:sendTextMessage(MESSAGE_STATUS_SMALL, "Você está recebendo mais experiência por dominar a guild-house.")
		end
	end
	...
	if self:getStorageValue(9992) >= os.time() then
		exp = exp * 1.10
		self:sendTextMessage(19, "Você está recebendo mais experiência por ter consumido o elixir.")
	end
	
	if self:getStorageValue(87221) >= 1 then
		exp = exp * 0.80
		self:sendTextMessage(MESSAGE_STATUS_DEFAULT, "Você está recebendo menos experiência por estar no modo NO-PVP.")
	end
	...
	if configManager.getBoolean(configKeys.STAMINA_SYSTEM) then
		...
		if staminaMinutes > 2400 then -- Se a stamina for maior que 40 horas (2400 min), XP normal
			exp = exp * 1.3
		else -- Se for menor que 40 horas, XP reduzido a 50%
			exp = exp * 1.0
		end
	end
