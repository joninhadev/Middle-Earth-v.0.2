local bossBurning = Action()

local leverStorage = 6180 -- Storage base para contar as alavancas ativadas
local teleportPosition = Position(411, 1309, 11)  -- Onde o teleport será criado
local teleportToPosition = Position(411, 1304, 11)  -- Para onde o teleport leva
local teleportItemId = 27721 -- ID do teleport
local totalLevers = 3
local teleportDuration = 600 -- Tempo que o teleport ficará ativo (5 minutos)
local resetLeversTime = 600 -- Tempo para resetar alavancas e expulsar jogadores (15 minutos após o teleport fechar)
local bossArea = {fromPos = Position(402, 1282, 11), toPos = Position(419, 1306, 11)}
local bossCooldownStorage = 8225 -- Storage para cooldown de 20 horas
local bossCooldownTime = 20 * 60 * 60 -- 20 horas em segundos
local bossName = "Ancalagon the Black"
local bossSpawnPosition = Position(409, 1287, 11)

-- **Posições das alavancas**
local leverPositions = {
    Position(354, 1381, 11),
    Position(408, 1183, 8),
    Position(180, 1049, 9)
}
...
function bossBurning.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if storageValue == totalLevers then
        ...
        if teleport then
            teleport:setActionId(9883) -- **Vincula o teleport ao MoveEvent**
            Game.broadcastMessage("As três alavancas foram movidas, Ancalagon the Black foi despertado!", MESSAGE_EVENT_ADVANCE)
...
bossBurning:aid(9829)
bossBurning:aid(9830)
bossBurning:aid(9831)
bossBurning:register()

-- **Verificação de entrada no teleport**
local tpBurning = MoveEvent()

function tpBurning.onStepIn(player, item, position, fromPosition)
