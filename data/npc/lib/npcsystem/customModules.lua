...
function Player.removeMoneyNpc(self, amount)
    ...
    if amount > moneyCount then
        ...
        if moneyCount == 0 then
            self:sendTextMessage(MESSAGE_INFO_DESCR, ("Pagou %d golds da sua conta bancaria. Seu saldo agora é %d golds."):format(amount, self:getBankBalance()))
        else
            self:sendTextMessage(MESSAGE_INFO_DESCR, ("Pagou %d do seu inventário e %d golds da sua conta bancaria. Sua seu saldo bancario agora ? %d golds."):format(moneyCount, amount - moneyCount, self:getBankBalance()))
        end
    end
    return true
end
